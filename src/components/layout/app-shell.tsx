...
const pageCopy: Record<string, { title: string; subtitle: string }> = {
  "/dashboard": { title: "Dashboard", subtitle: "Your next best study move" },
  ...
  "/mock-exams": { title: "Mock Exams", subtitle: "Practice likely exam questions" }
};
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-transparent text-[var(--text)]">
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 hidden border-r border-[var(--border)]/70 bg-[var(--surface)]/85 px-2 py-5 backdrop-blur-xl transition-[width] md:flex md:flex-col",
          collapsed ? "w-[72px]" : "w-[240px]"
        )}
      >
        <Link href="/dashboard" className="mb-6 flex items-center gap-3 px-2">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--primary)] to-[var(--accent)] text-white shadow-[0_14px_36px_rgba(107,124,255,0.28)]">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex min-h-11 items-center gap-3 rounded-2xl px-3 text-sm font-semibold text-[var(--muted)] transition hover:bg-[var(--subtle)] hover:text-[var(--text)]",
                  collapsed && "justify-center px-0",
                  active && "bg-[var(--subtle)] text-[var(--text)]"
                )}
              >
          ...
          {accountItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={cn("flex min-h-11 items-center gap-3 rounded-2xl px-3 text-sm font-semibold text-[var(--muted)] transition hover:bg-[var(--subtle)] hover:text-[var(--text)]", collapsed && "justify-center px-0")}
              >
                <Icon size={18} aria-hidden="true" />
                {!collapsed && <span>{item.label}</span>}
              </Link>
      ...
      <main className={cn("min-h-screen transition-[padding]", collapsed ? "md:pl-[72px]" : "md:pl-[240px]")}>
        <header className="sticky top-0 z-30 border-b border-[var(--border)]/70 bg-[var(--surface)]/80 px-3 py-3 backdrop-blur-xl sm:px-5">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-[var(--border)]/70 bg-[var(--surface)]/90 px-1 pb-[calc(env(safe-area-inset-bottom)+8px)] pt-2 backdrop-blur-xl md:hidden" aria-label="Mobile navigation">
        {navItems.map((item) => {
          const Icon = item.icon;
          const active = pathname === item.href;
          return (
            <Link key={item.href} href={item.href} className={cn("flex min-h-12 flex-col items-center justify-center gap-1 rounded-lg text-[10px] font-bold text-[var(--muted)]", active && "text-[var(--primary)]")}>
